...
| The lifecycle is strict and complete through D.5 | partially valid | Fresh, hash-bound B.1, B.2, and D.2 boards verify `4/4` each. A later retry against exact product snapshot `388845c` produced genuine Claude and GLM artifacts/receipts, but Gemini failed under a retired provider tier and the v2.0.1 schema/verifier defects remain. D.5 is warning-only fallback, not strict-P3 certified. |
...
| Reviewed-product-snapshot matrix, run `29155957357` | PASS — 7/7 jobs at exact head `388845c`; all Linux 3.9–3.12 jobs, non-editable install, and Windows 3.9/3.14 smoke checks green; full suite `1740 passed`, coverage `82.76%` |
| Product-snapshot D.5 retry | PROVIDER-LIMITED — Claude and GLM each produced a genuine artifact and round-2 receipt bound to `388845c` after one shape correction; Gemini's genuine retry failed exit `55` under the retired individual-client tier |
...
verified separately; they do not expand the `388845c` product-snapshot receipts.
`D4-INFRA-1` and `D5-INFRA-2` remain open. Maintainer-directed final report
label (not verifier output):

`provider_limited_fallback_complete; strict P3 not certified; maintainer release actions deferred`

The pinned framework's actual fallback result remains exit `1`,
`status=provider_limited_fallback_incomplete`; D.6 is `WITHHELD`.
