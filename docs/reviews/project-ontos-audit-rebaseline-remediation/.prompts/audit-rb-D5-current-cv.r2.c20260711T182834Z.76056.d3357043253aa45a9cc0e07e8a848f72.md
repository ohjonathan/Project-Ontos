...
You are not the orchestrator or fix author. Do not commit, stage, or spawn
agents. Your immediately preceding wrapper capture completed the requested
analysis but returned the artifact on stdout, which this route does not
promote. Do not rerun the test suite. Write only
`docs/reviews/project-ontos-audit-rebaseline-remediation/D.5-current-claude.md`
using the findings and command results from your preceding attempt, then emit a
brief completion notice. The framework wrapper validates and promotes that
single worker file.
...
Write `docs/reviews/project-ontos-audit-rebaseline-remediation/D.5-current-claude.md`
