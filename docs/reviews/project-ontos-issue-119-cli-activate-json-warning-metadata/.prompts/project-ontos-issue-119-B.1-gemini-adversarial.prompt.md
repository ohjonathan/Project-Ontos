# B.1 adversarial review — `project-ontos-issue-119-cli-activate-json-warning-metadata`

You are the **Adversarial Review** on Phase B.1 of deliverable `project-ontos-issue-119-cli-activate-json-warning-metadata` for repo `ohjonathan/Project-Ontos`. Family: `gemini`.

Adversarial reviewer (lens: 'How does this break?'). Attack the artifact under hostile or edge-case inputs (concurrency, schema-evasion, regressions). Do NOT duplicate Peer or Alignment.

This deliverable is a focused follow-up that proposes (a) adding a public `to_dict()` method on `ValidationError`, (b) swapping the CLI flattening at `ontos/commands/activate.py:104-105` from `[issue.message for ...]` to `[issue.to_dict() for ...]`, and (c) collapsing the MCP `_validation_issues` helper onto the same shared method. The change is a deliberate breaking change to the CLI JSON contract (`validation.warnings` and `validation.errors` element shape becomes `dict` instead of `str`). Attack the spec against: malformed `ValidationError` instances, concurrency/cache reuse, schema-evasion via empty strings, downstream consumer breakage, hidden mutation paths, and any regression of the existing MCP contract from PR #118.
