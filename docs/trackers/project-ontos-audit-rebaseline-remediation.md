...
| B.2 — corrected-spec board | Same engineering seats; Claude Product (separate session) | **reopened — v1.3 rerun pending** | Re-review corrected spec and I0 | Fresh v1.2 adversarial returned `Request changes` on X-1/X-2; spec v1.3 incorporates construction-level quarantine and literal-token tests; final four-seat rerun required |
| B.3 — design consolidation | Codex fast-path under unanimous external board | **reopened** | Canonical approved design verdict with every accepted finding and no hidden blocker | Original v1.2 consolidation preserved; recertification canonical refresh waits for final v1.3 B.2 board |
| C — code-first reconciliation | Independent Codex implementation-author worker | **in progress** | I0 and authorized deltas reconcile exactly to spec v1.3 | Original six work items plus recertification registry, log-marker, lock, copy, and test hardening |
