...
| B.2 — corrected-spec board | Same engineering seats; Claude Product (separate session) | **reopened — v1.5 final board pending** | Re-review corrected spec and I0 | Spec v1.5 incorporates all final B.1 should-fix/minor findings plus seven reproduced Phase C falsification gaps: eager version parsing, entry-binding races, control-plane roots/enums/ownership/paths, exact GitHub identity/checklists, and multi-link lock refusal. A fresh four-seat B.2 board is required. |
| B.3 — design consolidation | Codex fast-path under unanimous external board | **reopened** | Canonical approved design verdict with every accepted finding and no hidden blocker | Original v1.2 consolidation and v1.3 attempt evidence are preserved; canonical refresh waits for the final v1.5 B.2 board. |
| C — code-first reconciliation | Independent Codex implementation-author worker | **in progress — focused falsification fixes green** | I0 and authorized deltas reconcile exactly to spec v1.5 | Original work plus v1.5 fixes. Focused writer/lock/config/control-plane/log matrix passes; full clean-snapshot suite and C-close commit still required. |
