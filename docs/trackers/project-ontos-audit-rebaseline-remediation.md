...
| D.2 — implementation board | Claude peer; Gemini adversarial; GLM alignment; Product if declared | recertification rollover required | First four-seat bundle is complete and family-dispatch verified; strict lifecycle rejects a wrapper-generated GLM supersession backlink under route redaction, so immutable-receipt policy requires fresh B.1/B.2/D.2 round-1 bundles in a new inventory before D.3 | Attempt inventory and every capture remain preserved; no receipt is edited or reconstructed. |
