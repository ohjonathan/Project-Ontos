...
| B.1 — first design board | Claude adversarial; Gemini alignment; GLM peer; Claude Product (separate session) | **complete; recertified** | Complete hash-bound dispatch bundles and four verdicts | Fresh clean-inventory wrapper bundle 4/4 complete against spec v1.2; all four approve; original and failed attempt evidence preserved |
| B.2 — corrected-spec board | Same engineering seats; Claude Product (separate session) | **reopened — v1.4 final board pending** | Re-review corrected spec and I0 | The complete v1.3 board preserved two `Request changes` verdicts: Claude X-1/S-1 exposed untyped lease/integration consumers and an under-specified multi-clause diagnostic; GLM P-1 identified a misattributed lock-open anchor. Spec v1.4 closes those items and the GLM diagram/anchor clarity minors; a fresh final board is required. |
| B.3 — design consolidation | Codex fast-path under unanimous external board | **reopened** | Canonical approved design verdict with every accepted finding and no hidden blocker | Original v1.2 consolidation and v1.3 attempt evidence are preserved; canonical refresh waits for the final v1.4 B.2 board. |
| C — code-first reconciliation | Independent Codex implementation-author worker | **in progress** | I0 and authorized deltas reconcile exactly to spec v1.4 | Original six work items plus recertification registry, log-marker, lock, copy, and test hardening; v1.4 lease/integration quarantine and multi-clause regressions added. |
