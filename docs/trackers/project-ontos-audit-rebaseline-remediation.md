...
| D.4 — fixes | Codex fix-author | **functional fixes complete at I2; halted on D4-INFRA-1** | Per-ID fix summary, test-first evidence, scope proof, and honest framework-blocker disposition | `D.4-fix-summary.md`; I2 `311b60b`; all six D.3 findings functionally addressed. Focused matrices `124`, `205`, and `109` passed; full suite `1720 passed`; live registry parity and 559-path scope pass. Framework v2.0.1 EH-15-A cannot register adopter-local fixtures and its manifest path lookup is fail-open, so D.4 is not certified. |
