...
- **`ontos env`** — Detects environment manifests in your repository:
  - `pyproject.toml`, `Brewfile`, `package.json`, `.tool-versions`, `requirements.txt`, and more.
- **`ontos doctor` environment check** — Now includes a 9th check that warns about missing or unparseable environment manifests.
- **Candidate suggestions** — Suggests documents to scaffold based on detected environments.

### Theme 3: Activation Resilience

- **Tiered context map** — New 3-tier structure replaces the flat context map:
  - **Tier 1 (~2k tokens)**: Project summary, recent activity, critical paths — enough for AI orientation.
  - **Tier 2**: Full document index with type and status.
  - **Tier 3**: Complete dependency graph, timelines, and health metrics.
- **`ontos map --sync-agents`** — Automatically regenerates AGENTS.md alongside the context map.
- **AGENTS.md Current Project State** — Shows branch, doc count, last log, and health status.
- **AGENTS.md Re-Activation Trigger** — Context recovery hints for when AI agents lose orientation.
- **USER CUSTOM section preservation** — Custom sections in AGENTS.md are preserved during regeneration using explicit markers.
...
- `gather_stats` optimized — only scans `docs/` and `logs/` directories, not the entire repo.
