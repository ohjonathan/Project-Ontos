...
- All 7 CLI commands now run **natively** — no more subprocess overhead.
...
- **`ontos map --obsidian`** — Generates Obsidian-compatible wiki-links (`[[doc_id]]`) in the context map, enabling direct integration with Obsidian vaults.
- **`ontos map --compact`** — Reduced-token output mode for LLM context windows. Strips verbose formatting while preserving all semantic content.
- **`ontos map --filter`** — Type-based filtering (e.g., `--filter strategy,kernel`) to generate focused context maps.
- **Token-aware caching** — Caches map generation results with a 15-minute TTL. Subsequent runs within the window return cached output instantly.
- **`ontos doctor -v`** — Verbose diagnostics mode showing detailed check results.
...
- **`scaffold` no longer rejects positional arguments** — Was broken since v3.0 due to argparse migration.
- **`consolidate` import crash on missing config defaults** (B1) — Fixed missing default values.
- **`promote` crash on absolute paths outside repo** (B2) — Now handles edge case gracefully.
