...
def mock_docs():
    ...
    for i in range(1, 10):
        ...
        docs[log_id] = DocumentData(
            ...
            status=DocumentStatus.ACTIVE,
            frontmatter={"summary": f"Log entry {i}"},
...
def test_agents_user_custom_preservation(tmp_path, monkeypatch):
    ...
    custom_content = "\n## My Rules\n1. Always test."
    new_content = original_content.replace("# USER CUSTOM", "# USER CUSTOM" + custom_content)
    ...
    assert "Generated by Ontos" in final_content # Verification that it was indeed regenerated
...
def test_tier1_token_capping(mock_docs):
    ...
    # Let's add more logs to be sure
    for i in range(10, 20):
        log_id = f"log_large_{i}"
        # Positional: id, type, status, filepath, frontmatter, content
        mock_docs[log_id] = DocumentData(
            log_id,
            DocumentType.LOG,
            DocumentStatus.ACTIVE,
            Path(f"logs/{log_id}.md"),
