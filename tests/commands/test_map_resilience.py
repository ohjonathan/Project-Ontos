...
def test_map_sync_agents_flag(tmp_path, monkeypatch):
    """Should sync an Ontos-owned AGENTS.md when --sync-agents is used,
    preserving the USER CUSTOM section."""
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".ontos.toml").write_text("[ontos]\nversion = '3.2'")
    (tmp_path / ".ontos-internal").mkdir()
    (tmp_path / "docs").mkdir()
    ...
    agents_file = tmp_path / "AGENTS.md"
    # Ontos-owned (generation header present) but stale content with a
    # user-custom section that must survive regeneration.
...
def test_map_sync_agents_skips_managed_block_file(tmp_path, monkeypatch):
    """(#173) A file with USER CUSTOM markers but no Ontos generation header
    is not Ontos-owned; --sync-agents must leave it byte-identical."""
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".ontos.toml").write_text("[ontos]\nversion = '3.2'")
    (tmp_path / ".ontos-internal").mkdir()
    (tmp_path / "docs").mkdir()
    ...
    original = (
        ...
        "<!-- USER CUSTOM -->Custom bits<!-- /USER CUSTOM -->\n## My Tail"
    )
