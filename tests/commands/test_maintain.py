...
def test_maintain_consolidate_self_heals_generated_narrative_history(
    ...
    for day in (1, 2):
        (logs_dir / f"2020-01-0{day}-log-{day}.md").write_text(
            "---\n"
            f"id: log_{day}\n"
            "type: log\n"
            "event_type: chore\n"
    ...
    history_path = tmp_path / "docs" / "strategy" / "decision_history.md"
    ...
    result = _task_consolidate_logs(ctx)

    history = history_path.read_text(encoding="utf-8")
    assert result.status == "success"
    assert result.exit_code == 0
    assert result.message == "Consolidated 1 logs"
    assert result.metrics == {"retention_count": 1}
    assert history.startswith(original_history)
    ...
    assert len(list((tmp_path / "docs" / "archive" / "logs").glob("*.md"))) == 1


def test_agents_staleness_is_content_based_not_mtime(tmp_path, monkeypatch):
    """(#173) Fresh generated content is never stale, regardless of mtimes —
    the clone/worktree case where checkout write order used to flip the
    verdict."""
    _init_project(tmp_path)
    monkeypatch.chdir(tmp_path)
    ctx = _build_context(tmp_path)
    ...
    # Every source newer than AGENTS.md: the old mtime heuristic said
    # "stale"; content comparison says fresh.
    ...
    # Real content drift (project state changed: a new doc) → stale even
    # though AGENTS.md now has the NEWEST mtime.
    (tmp_path / "docs" / "new-doc.md").write_text(
        "---\nid: new_doc\ntype: atom\nstatus: active\n---\n", encoding="utf-8"
    )
    os.utime(agents_path, None)
    should_run, reason = _condition_agents_stale(ctx)
    assert should_run is True
    assert "stale" in reason.lower()


def test_agents_staleness_skips_user_managed_file(tmp_path, monkeypatch):
    """(#173) A hand-authored AGENTS.md is never reported stale and never
    regenerated by the sync task — the reproduced data-loss trap."""
    _init_project(tmp_path)
    monkeypatch.chdir(tmp_path)
    ctx = _build_context(tmp_path)
    agents_path = tmp_path / "AGENTS.md"
    hand_authored = "# My own agent instructions\n\nDo not touch.\n"
    agents_path.write_text(hand_authored, encoding="utf-8")

    should_run, reason = _condition_agents_stale(ctx)
    assert should_run is False
    assert "not ontos-managed" in reason.lower()
    assert "user_managed" in reason

    # Defense in depth: even if the task itself is forced to run, it must
    # not overwrite the file.
    result = _task_sync_agents(ctx)
    assert result.status == "success"
    assert "not Ontos-managed" in result.message
    assert agents_path.read_text(encoding="utf-8") == hand_authored
    assert not (tmp_path / "AGENTS.md.bak").exists()


def test_sync_agents_noop_keeps_bytes_for_fresh_content(tmp_path, monkeypatch):
    """A semantic no-op leaves AGENTS.md bytes untouched (no .bak, no touch)."""
    _init_project(tmp_path)
    monkeypatch.chdir(tmp_path)
    ctx = _build_context(tmp_path)
    agents_path = tmp_path / "AGENTS.md"
    ...
    agents_path.write_text(content, encoding="utf-8")
    os.utime(agents_path, (100, 100))
    before = agents_path.stat().st_mtime
    ...
    assert not (tmp_path / "AGENTS.md.bak").exists()
    # (#173) The touch_on_unchanged mtime hack is retired: bytes AND
    # metadata stay untouched on a no-op.
    assert agents_path.stat().st_mtime == before
