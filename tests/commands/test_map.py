...
def test_map_sync_agents_updates_ontos_owned_agents_md(tmp_path, monkeypatch):
    """When --sync-agents is set and AGENTS.md is Ontos-owned, it updates."""
    # Setup: Create minimal ontos project
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".ontos.toml").write_text("[project]\nname = 'test'\n")
    (tmp_path / ".ontos-internal").mkdir()

    # Create a stale but Ontos-owned AGENTS.md (carries the generation
    # header, so the (#173) ownership guard allows the sync).
    agents_path = tmp_path / "AGENTS.md"
    agents_path.write_text(
        "# AGENTS.md\n\nGenerated by Ontos v0.0.0 on 2020-01-01 00:00:00 UTC\n\n# Old content\n"
    )
...
def test_map_sync_agents_never_overwrites_user_managed_file(tmp_path, monkeypatch):
    """(#173) --sync-agents must skip an AGENTS.md Ontos did not generate."""
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".ontos.toml").write_text("[project]\nname = 'test'\n")
    (tmp_path / ".ontos-internal").mkdir()
    (tmp_path / ".git").mkdir()
    ...
    agents_path.write_text(hand_authored)

    map_command(MapOptions(sync_agents=True))

    assert agents_path.read_text() == hand_authored
