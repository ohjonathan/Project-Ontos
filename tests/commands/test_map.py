...
def test_map_vocabulary_loader_tolerates_invalid_utf8(tmp_path):
    reference = tmp_path / ".ontos-internal" / "reference"
    reference.mkdir(parents=True)
    (reference / "Common_Concepts.md").write_bytes(b"\xff\xfe")

    from ontos.commands.map import _load_known_concepts

    assert _load_known_concepts(tmp_path) == set()
...
# --- #136: generator provenance in the map header ---

def test_generate_header_renders_installed_version_and_metadata():
    """Header must report the installed package version, not a config string."""
    from ontos import __version__
    from ontos.commands.map import _generate_header

    header = _generate_header({"project_name": "Demo", "scope": "docs"}, document_count=7)

    assert f"> Auto-generated by Ontos {__version__}" in header
    assert f"generator_version: {__version__}" in header
    assert "scope: docs" in header
    assert "documents_loaded: 7" in header
    # Map schema version is independent of the package version.
    assert "ontos_map_version: 2" in header


def test_map_command_writes_generator_metadata(tmp_path, monkeypatch):
    """End-to-end: generated map frontmatter carries provenance fields."""
    from ontos import __version__
    ...
    assert str(frontmatter["generator_version"]) == __version__
    assert frontmatter["scope"] == "docs"
    assert frontmatter["documents_loaded"] == 1
    assert frontmatter["ontos_map_version"] == 2
    # The config-format version ("3.0") must not be displayed as the product.
    assert "Auto-generated by Ontos 3.0" not in content
    assert f"Auto-generated by Ontos {__version__}" in content


def test_map_custom_output_is_excluded_from_its_own_scan(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".ontos.toml").write_text(
        "[ontos]\nversion = '3.0'\n",
        encoding="utf-8",
    ...
    custom_output = docs / "custom-map.md"
    custom_output.write_text(
        "---\nid: stale_generated_map\ntype: strategy\nstatus: complete\n---\n",
        encoding="utf-8",
    )

    exit_code = map_command(MapOptions(output=custom_output, quiet=True))

    assert exit_code == 0
    assert "documents_loaded: 1" in custom_output.read_text(encoding="utf-8")


# --- #131: map --json result_status + grouped diagnostics ---

def _run_map_json(tmp_path, *extra):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "ontos", "--json", "map", *extra],
        cwd=tmp_path,
        capture_output=True,
        text=True,
        env=env,
    )
    return result, _json.loads(result.stdout)


def test_map_json_clean_repo_reports_clean_status(tmp_path):
    (tmp_path / ".ontos.toml").write_text("[ontos]\nversion = '3.0'\n")
    (tmp_path / "docs").mkdir()
    (tmp_path / "docs" / "a.md").write_text("---\nid: a\ntype: atom\nstatus: active\n---\n")

    result, envelope = _run_map_json(tmp_path)

    assert result.returncode == 0
    data = envelope["data"]
    assert data["result_status"] == "clean"
    assert data["strict"] is False
    assert data["diagnostics"] == []


def test_map_strict_json_carries_grouped_diagnostics(tmp_path):
    (tmp_path / ".ontos.toml").write_text("[ontos]\nversion = '3.0'\n")
    (tmp_path / "docs").mkdir()
    # A kernel orphan (not in default allowed_orphan_types) -> strict warning.
    ...
    groups = data["diagnostics"]
    assert groups, "strict failure must carry grouped diagnostics, not bare counts"
    orphan_group = next(g for g in groups if g["rule_id"] == "orphan")
    assert orphan_group["count"] >= 1
    assert orphan_group["samples"]
