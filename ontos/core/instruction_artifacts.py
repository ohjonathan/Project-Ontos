...
def generate_agents_content(
    ...
    (#173, PR #182 review) ``repo_root`` pins which repository the stats
    describe; falling back to CWD discovery is only for legacy callers.
    Freshness comparisons MUST pass it explicitly, or a process running in
    another repository would compute cross-repository "staleness".
    """
...
# (#173, PR #182 review) Ownership must be proven by the exact generated
# header line in its fixed structural position — never by the phrase
# appearing anywhere (e.g. quoted as an example inside a hand-authored
# file). Structural validation: the header must sit within the template
# head, and every non-blank line before it must itself be a known template
# head line. Files that fail the check fall to user_managed — the safe
# direction (skipped, never overwritten; adoptable via `ontos agents
# --force`).
_GENERATED_HEADER_LINE_RE = re.compile(
    r"^Generated by Ontos v\S+ on \d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} UTC$"
)
_OWNERSHIP_HEADER_SCAN_LINES = 8
_OWNERSHIP_TEMPLATE_HEAD_LINES = {
    "# AGENTS.md",
...
def _has_structural_generation_header(content: str) -> bool:
    """True when the generated header occupies its fixed template position."""
    head = content.splitlines()[:_OWNERSHIP_HEADER_SCAN_LINES]
    for line in head:
        if _GENERATED_HEADER_LINE_RE.fullmatch(line):
            return True
        if line.strip() and line not in _OWNERSHIP_TEMPLATE_HEAD_LINES:
            # Arbitrary content precedes the header: this is user prose
            # quoting the phrase, not the generated template head.
            return False
    return False
...
def classify_instruction_ownership(path: Path) -> str:
    ...
    Returns one of:
    - ``ontos_owned``: the exact ``Generated by Ontos v<version> on
      <timestamp> UTC`` header line appears, unindented and byte-exact, in
      its fixed template position — within the file head, preceded only by
      known template head lines — so the whole file is an Ontos-generated
      artifact (its USER CUSTOM section is preserved on regeneration). The
      phrase appearing anywhere else (quoted example, code block, prose
      after user content) does NOT confer ownership.
    - ``managed_block``: no generation header, but contains USER CUSTOM
      markers — treated exactly like ``user_managed`` for write safety;
      the label only aids diagnostics.
    ...
    if _has_structural_generation_header(content):
...
def is_agents_semantically_stale(
    ...
    expected = generate_agents_content(existing, scope=scope, repo_root=repo_root)
...
# (#173, PR #182 review) The branch is checkout-local state, not semantic
# project content: two worktrees at the same HEAD must agree on freshness.
_BRANCH_ROW_RE = re.compile(r"^\| Branch \| .* \|$")


def _without_generated_instruction_timestamps(content: str) -> str:
    """Normalize checkout-volatile instruction lines for semantic comparison.

    Timestamps and the current-branch row are checkout/host state, not
    project content; they must not drive staleness verdicts or rewrites.
    (The regenerated file still records the live values whenever a real
    content change triggers a write.)
    """
    normalized = []
    for line in content.splitlines(keepends=True):
        ...
        elif _BRANCH_ROW_RE.fullmatch(value):
    ...
    if generate_agents:
        ...
        try:
            ...
            agents_content = generate_agents_content(
                existing_content, scope=scope, repo_root=repo_root
            )
