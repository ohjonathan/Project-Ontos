...
def create_parser() -> argparse.ArgumentParser:
    ...
    # Hide deprecated/internal commands from top-level help while preserving
    # direct invocation for backwards compatibility.
...
def _hide_subcommands_in_help(subparsers, hidden_commands: Iterable[str]) -> None:
    """Hide selected subcommands from top-level argparse help output."""
    hidden = set(hidden_commands)

    # Hide from command listing table.
    subparsers._choices_actions = [
        action for action in subparsers._choices_actions
        if action.dest not in hidden
    ]

    # Hide from usage line by replacing autogenerated metavar.
    visible_commands = [name for name in subparsers.choices if name not in hidden]
