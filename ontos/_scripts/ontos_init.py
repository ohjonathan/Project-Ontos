...
v3.0: Uses bundled templates and hooks from package (no .ontos/ copy required).

This script will:
1. Configure Ontos mode interactively
2. Create optional directories (.ontos-internal, docs)
3. Install git hooks (from bundled package)
...
def get_templates_dir() -> str:
    """Get path to bundled templates directory (ontos/_templates/)."""
    return str(Path(__file__).resolve().parent.parent / "_templates")


def get_hooks_dir() -> str:
    """Get path to bundled hooks directory (ontos/_hooks/)."""
    return str(Path(__file__).resolve().parent.parent / "_hooks")
...
def generate_config(mode: str, source: str) -> None:
    ...
    """
    # v3.0: Use bundled template from package
    template_path = os.path.join(get_templates_dir(), 'ontos_config.py.template')
...
# Import defaults (required) - v3.0: from bundled package
...
def scaffold_starter_docs() -> None:
    ...
    v3.0: Uses bundled template functions from ontos/_templates/.
    Respects DOCS_DIR config setting. Idempotent - safe to run multiple times.
    """
    # Get docs dir from config
    try:
    ...
    # v3.0: Import template functions from bundled package
    try:
        from ontos._templates.templates import get_decision_history_template, get_common_concepts_template
    except ImportError:
...
def install_pre_commit_hook() -> bool:
    ...
    """
    # v3.0: Use bundled hooks from package
    pre_commit_src = os.path.join(get_hooks_dir(), 'pre-commit')
...
def main():
    ...
    print("══════════════════════════════════════════════════════════════")
    print("             Welcome to Project Ontos v3.0.0 Setup")
    print("══════════════════════════════════════════════════════════════")
    ...
    # Install pre-push hook (v3.0: from bundled package)
    bash_hook_src = os.path.join(get_hooks_dir(), 'pre-push')
