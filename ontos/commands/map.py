...
def generate_context_map(
    ...
    sections = [
        _generate_header(config, document_count=len(docs)),
...
def _generate_header(config: Dict[str, Any], document_count: int = 0) -> str:
    """Generate context map header with provenance and frontmatter.

    `ontos_map_version` is the map *schema* version (the layout contract of
    this file). It is independent of `generator_version`, which records the
    installed package version that produced the map (#136).
    """
...
> Auto-generated by Ontos {ONTOS_VERSION}
