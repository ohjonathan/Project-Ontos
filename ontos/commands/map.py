...
def generate_context_map(
    ...
    # Tier 1: Activation Context (Recent Work + Architecture + Errors)
    tier1_sections = []
    tier1_sections.append(_generate_header(config))
    tier1_sections.append(_generate_tier1_recent_logs(docs))
    tier1_sections.append(_generate_tier1_architecture(docs, options.obsidian))
    if result.errors:
        tier1_sections.append(_generate_validation_section(result, errors_only=True))

    tier1_content = "\n\n".join(filter(None, tier1_sections))
    
    # Token Cap for Tier 1 (approx 2000 tokens)
    # 1 token ~= 4 chars, so 2000 tokens ~= 8000 chars
    # We use estimate_tokens for accuracy
    tier1_tokens = estimate_tokens(tier1_content)
    if tier1_tokens > 2000:
        # Truncate and add overflow message
        tier1_content = tier1_content[:8000] + "\n\n... (Tier 1 truncated, see full map below) ..."

    # Tier 2: Deep Context (Full Table + Warnings + Tree + Stats)
    tier2_sections = ["---", "# Tier 2: Deep Context"]
    tier2_sections.append(_generate_document_table(docs, options.obsidian))
    
    if result.errors or result.warnings:
        tier2_sections.append(_generate_validation_section(result))

    tier2_sections.append(_generate_dependency_tree(docs))

    if options.include_timeline:
        tier2_sections.append(_generate_timeline(docs))

    if options.include_staleness:
        staleness_section = _generate_staleness_section(docs)
        if staleness_section:
            tier2_sections.append(staleness_section)

    if options.include_lint:
        lint_section = _generate_lint_section(docs, result)
        if lint_section:
            tier2_sections.append(lint_section)

    tier2_content = "\n\n".join(filter(None, tier2_sections))

    # Assemble content
    content = f"{tier1_content}\n\n{tier2_content}"
...
This document provides a tiered index of the knowledge graph.
- **Tier 1 (Top)**: Recent work, architecture, and errors (high priority).
- **Tier 2 (Bottom)**: Full document index and deep graph analysis."""
...
def _generate_tier1_recent_logs(docs: Dict[str, DocumentData]) -> str:
    """Generate Tier 1 recent logs section (limit to 5)."""
    logs = [d for d in docs.values() if d.type.value == "log" or str(d.type) == "log"]
    if not logs:
        return ""
        
    sorted_logs = sorted(logs, key=lambda d: d.id, reverse=True)[:5]
    
    lines = ["## Tier 1: Recent Session Logs", ""]
    for log in sorted_logs:
        summary = log.frontmatter.get("summary", "No summary")
        lines.append(f"- `{log.id}`: {summary}")
        
    return "\n".join(lines)


def _generate_tier1_architecture(docs: Dict[str, DocumentData], obsidian_mode: bool) -> str:
    """Generate Tier 1 architecture section (strategy, reference)."""
    arch_types = ["strategy", "reference"]
    arch_docs = [d for d in docs.values() 
                 if (d.type.value if hasattr(d.type, 'value') else str(d.type)) in arch_types]
    
    if not arch_docs:
        return ""
        
    lines = ["## Tier 1: Core Architecture", ""]
    # Sort by type then ID
    sorted_arch = sorted(arch_docs, key=lambda d: (str(d.type), d.id))
    
    for doc in sorted_arch:
        doc_type = doc.type.value if hasattr(doc.type, 'value') else str(doc.type)
        summary = doc.frontmatter.get("summary", "No summary")
        doc_link = _format_doc_link(doc.id, doc.filepath, obsidian_mode)
        lines.append(f"- {doc_link} ({doc_type}): {summary}")
