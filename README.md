...
Ontos also recognizes lifecycle artifact types for real engineering workspaces:
`handoff`, `tracker`, `retro`, `review`, `spec`, `report`, `adr`, and
`policy`. Workflow statuses such as `proposed`, `ready`, `completed`,
`revised`, and `in-lifecycle` are accepted so review boards, release gates, and
handoff packets keep their meaning instead of collapsing to `unknown`.
