...
type: strategy
status: complete
depends_on: [v2_7_1_implementation_plan]
...
| 2.3.0 | 2025-12-22 | v2.8.5 completed (A+); added v2.8.6 Agent Instructions update |
...
|-------------|------------|---------|--------|
| `ontos.py log` | `ontos_end_session.py` | `archive`, `session` | ✅ v2.8.5 |
| `ontos.py map` | `ontos_generate_context_map.py` | `context`, `generate` | ✅ v2.8.5 |
| `ontos.py verify` | `ontos_verify.py` | `check` | ✅ v2.8.5 |
| `ontos.py init` | `ontos_init.py` | `setup` | 🔲 Deferred to v3.0 |
| `ontos.py maintain` | `ontos_maintain.py` | `maintenance` | ✅ v2.8.5 |
| `ontos.py consolidate` | `ontos_consolidate.py` | `archive-old` | ✅ v2.8.5 |
| `ontos.py query` | `ontos_query.py` | `search`, `find` | ✅ v2.8.5 |
| `ontos.py update` | `ontos_update.py` | `upgrade` | ✅ v2.8.5 |
...
|---------|-----|-------------|-------|--------|
| v2.8.1 | #25 | Refactor `ontos_generate_context_map.py` | A- | ✅ Merged |
| v2.8.2 | #26 | Begin `ontos_end_session.py` refactoring | A- | ✅ Merged |
| v2.8.3 | #27 | Complete `ontos_end_session.py` to A+ | A+ | ✅ Merged |
| v2.8.4 | #28 | Refactor remaining scripts | A+ | ✅ Merged |
| v2.8.5 | #29 | Unified CLI (`ontos.py`) | A+ | ✅ Merged |
| **v2.8.6** | **#30** | **Agent Instructions update** | Target: A+ | 🔲 In Progress |

---

### 12.2 v2.8.1: ontos_generate_context_map.py (PR #25) ✅
...
### 12.3 v2.8.2: ontos_end_session.py Initial (PR #26) ✅
...
|----------|-------|--------------|--------------|--------|
| `create_log_file()` | 1278-1373 | ✅ | ✅ | Fix commit timing |
| `_create_archive_marker()` | 1373-1395 | — | ✅ | Receive ctx from caller |
| `graduate_proposal()` | 123-200 | ✅ | ❌ | **Convert** |
| `append_to_log()` | 359-430 | ✅ | ❌ | **Convert** |
| `create_auto_log()` | 600-690 | ✅ | ❌ | **Convert** |
| `create_changelog()` | 922-960 | ✅ | ❌ | **Convert** |
| `add_to_decision_history()` | 230-280 | ❌ | ❌ | **Convert** |
| `update_changelog()` | 1000-1060 | ❌ | ❌ | **Convert** |
...
1. **Update create_log_file()** — Add `_owns_ctx` check, pass ctx to `_create_archive_marker()`
2. **Update main()** — Single commit at end after all buffered writes
...
### 12.5 v2.8.4: Remaining Scripts (PR #28) ✅

**Status:** Merged
**Grade:** A+

**Scripts Refactored:**
- `ontos_consolidate.py` (21 prints → OutputHandler, 1 write → buffer_write)
- `ontos_verify.py` (20 prints → OutputHandler, 1 write → buffer_write)
- `ontos_maintain.py` (25 prints → OutputHandler, 2 writes → buffer_write, **deduplication**)
- `ontos_query.py` (16 prints → OutputHandler, read-only - no writes)
...
Required tests:
- `test_help_flag()` — `python3 ontos.py --help` returns 0, shows usage
- `test_version_flag()` — `python3 ontos.py --version` shows version
- `test_unknown_command()` — Unknown command returns 1 with error
- `test_alias_resolution()` — `archive` → `log`, etc.
- `test_each_command_has_help()` — Each command responds to `--help`
- `test_command_passthrough()` — Arguments passed to subcommand correctly

**Task E: Update Documentation [MEDIUM]**

Files to update:
- `docs/reference/Ontos_Manual.md` — Add unified CLI syntax examples
- `docs/reference/Ontos_Agent_Instructions.md` — Update command reference
- `Ontos_CHANGELOG.md` — Add v2.8 section with all changes
...
### 12.7 v2.8.6: Agent Instructions Update (PR #30) 🔲

**Status:** In Progress
**Target Grade:** A+

**Purpose:** Documentation alignment — update Agent Instructions to use unified CLI syntax.

**Changes:**
- 11 command references updated from old script paths to `python3 ontos.py <command>`
...
**Success Criteria:**
- [x] All 11 old-style commands replaced with unified CLI
- [x] Unified CLI note added at top
- [x] Version bumped to 2.8.6
- [x] CHANGELOG updated
- [ ] All tests pass (285+)
- [ ] PR #30 created
...
*End of v2.8 Implementation Plan v2.3.0*

*APPROVED by LLM Review Board (Claude, Codex, Gemini) on 2025-12-20.*
*v2.8.5 implementation complete. v2.8.6 in progress.*
