...
type: atom
status: complete
depends_on: [v3_0_3_hybrid_review_peer_gemini, v3_0_2_hybrid_review_alignment_claude, v3_0_3_hybrid_review_adversarial_codex]
...
**Date:** 2026-01-18  
**Role:** Consolidator  
**Phase:** B+D Hybrid — Consolidation
...
## 2. "Already Fixed" Claims — Consolidated Verdict
...
|-------|------------------|------------------|-----------------|---------------|
| Init uses native map_command | ❌ **FALSE** — uses subprocess lines 135-141 | ✅ Verified — uses `from ontos.commands.map import map_command` | N/A | ⚠️ **DISPUTED** |
| Map command works | ❌ **FALSE** — subprocess lines 249-253 | ✅ Verified — `_cmd_map()` calls native | N/A | ⚠️ **DISPUTED** |
| Common_Concepts resolved | ✅ Verified | ✅ Confirmed | N/A | ✅ **Confirmed** |
| Uninstall docs correct | ✅ Verified | ✅ Confirmed | N/A | ✅ **Confirmed** |
...
| **B3** | `sys.path.insert(0, ...)` shadows package `ontos` | Codex | Runtime Regression | Proposed fix causes `ModuleNotFoundError: ontos.core` — commands fail |
...
| NB3 | Scope change (6→4 files) lacks paper trail | Claude | Could | Add note explaining already-implemented changes |
...
|---------------|---------|------------|------------------|
| Init ModuleNotFoundError | 80-81 | CA: "Implicitly fixed" | ⚠️ Disputed |
| Map ModuleNotFoundError | 92-93 | CA: "WORKS NOW" | ⚠️ Disputed |
| Context map returns code 1 | 107-109 | ✅ Native command | ✅ All agree |
| `.ontos.toml` ignored | 499-503 | ❌ **NOT ADDRESSED** | ✅ Claude flagged |
| Shim workaround needed | 297-306 | N/A (user mitigation) | ✅ All agree |
...
|-------|-------------|-------------|------------|------------|
| **Init/Map native?** | ❌ Uses subprocess (lines 135, 249) | ✅ Native (lines 104, 240) | N/A | **Must resolve** — check current code |
...
| **3** | **Fix the fix**: The proposed `sys.path.insert(0, ...)` shadows `ontos` package — revise approach (e.g., insert after package root, or use explicit imports) | B3 |
...
**Spec Status:** ❌ **Revision Required**

**Reasons:**
1. **Factual dispute** — Two reviewers provide contradictory evidence for the same code. Cannot approve without resolution.
2. **Missing bug log item** — `.ontos.toml` ignored is a documented user issue with no spec response.
3. **Fix breaks commands** — Adversarial testing shows the proposed fix introduces new runtime failures.
