---
id: v3_3_track_b_implementation_spec
type: strategy
status: draft
depends_on:
  - v3_3_release_plan
...
# v3.3 Track B Implementation Spec
...
- `--quiet`: suppress non-essential informational lines (human mode only).
...
- `impacts`/`describes`: `ValidationOrchestrator.validate_impacts()` and `validate_describes()` with severity override to `error` for link-check.
- Suggestions: `core/suggestions.py:suggest_candidates_for_broken_ref(...)` for each broken reference.

Important: link-check does **not** claim concepts validation coverage (VUL-06 caveat).
...
- Valid boundaries before/after match: whitespace, `[`, `]`, `(`, `)`, `` ` ``, `,`, start-of-line/file, end-of-line/file.
...
- `start == 0` or `text[start-1] in BOUNDARY`
- `end == len(text)` or `text[end] in BOUNDARY`
...
- Extract `target` only (label is not analyzed for references).
...
2. Summary
   - duplicates, broken frontmatter refs, broken body refs, orphan count, exit code.
3. Duplicate IDs
...
| `broken_references[].location` | Positional metadata for body matches; omitted for pure frontmatter refs. |
...
| Duplicates | Broken refs | Orphans | Exit |
|-----------|-------------|---------|------|
| 0 | 0 | 0 | 0 |
| 1+ | any | any | 1 |
| 0 | 1+ | any | 1 |
...
- `maintain` task `check_links` calls the same function (S5 consolidation requirement), with `include_body=False` for parity/performance unless later expanded.
...
1. Clean git state check
   ...
   - fail if any output.
2. `old_id` exists exactly once as explicit frontmatter `id`.
3. `new_id` does not exist as explicit frontmatter `id`.
4. No duplicate IDs in scope (`load_result.duplicate_ids` must be empty; B5).
5. Parse integrity for rewrite candidates
   - canonical parser only (`parse_frontmatter_content`, S4)
   - files with parse failure are skipped with warnings; if any skipped file contains a potential rename target occurrence discovered by raw scan, abort for safety.
6. Full rewrite plan built in memory
   - all frontmatter and body edits computed and conflict-checked.

No write is buffered before step 6 completes.
...
1. Split file into three parts only if frontmatter exists at file start:
   - opening delimiter line
   - raw frontmatter block (unchanged order/comments)
   - body
2. Parse frontmatter dict via canonical parser for semantic decisioning.
3. For each target field (`id`, `depends_on`, `impacts`, `describes`) patch only matching values in raw frontmatter block.
...
- mark file as unsupported-target-format,
...
4. On success, print summary and exact post-apply warning:
...
- Because command requires clean git state, recovery path is deterministic (`git restore`/`git checkout` by user policy).
...
5. dirty git state is rejected before planning/writing.
...
- Mitigation:
  ...
  - explicit boundary/token rules (B1),
...
- `io/files.py:load_frontmatter()` for single-file parse contract.
...
- If touching `graph.py`/`suggestions.py`, remove or justify residual `hasattr(...)` checks per VUL-07 decision in implementation phase.
