...
- **Map provenance tracking** in generated context maps
- **Release packaging exclusions** to prevent internal docs shipping to users
...
### 6.4 Map Provenance Check

**Risk identified by Codex:** Contributor's environment is misconfigured, they generate a map in User Mode (scanning empty `docs/` instead of `.ontos-internal/`), commit it, and the repo ships with a useless context map.

**Solution:** Add a metadata header to the generated map indicating its provenance.
...
**Implementation in `ontos_generate_context_map.py`:**

```python
def get_mode_name() -> str:
    """Return the current operating mode for provenance tracking."""
    if is_ontos_repo():
        return "Contributor"
    return "User"
...
```yaml
# .github/workflows/ontos.yml
- name: Verify map was generated in Contributor Mode
  run: |
    grep -q "Mode: Contributor" Ontos_Context_Map.md || {
      echo "ERROR: Context map was generated in User Mode"
      exit 1
    }
```

**Benefits:**
- Self-documenting (anyone can see what mode produced the map)
- CI can assert expected mode
- No schema changes, just output formatting
- Timestamp aids debugging stale map issues
...
## Part 8: Release Packaging Hygiene

**Risk identified by Codex:** `.ontos-internal/` (and logs) could accidentally ship in distribution artifacts, causing user installations to receive internal docs even if skip patterns are in place.

### 8.1 What Should Never Ship

| Artifact Type | Excluded Items |
|---------------|----------------|
| PyPI package (sdist/wheel) | `.ontos-internal/`, `Ontos_Context_Map.md` (generated) |
| GitHub release zip | `.ontos-internal/`, `.ontos-internal/logs/` |
| Documentation site | Internal strategy/architecture docs |

### 8.2 Exclusion Methods (Defense in Depth)

**A. Git archive exclusion (GitHub release zips):**

```gitattributes
# .gitattributes
.ontos-internal/ export-ignore
Ontos_Context_Map.md export-ignore
```

**B. Python sdist exclusion (MANIFEST.in):**

```
# MANIFEST.in
prune .ontos-internal
exclude Ontos_Context_Map.md
```
...
"*" = [".ontos-internal/*", "Ontos_Context_Map.md"]
```

**D. Hatch/Flit (if using):**

```toml
# pyproject.toml (hatch)
[tool.hatch.build.targets.sdist]
exclude = [".ontos-internal/", "Ontos_Context_Map.md"]

[tool.hatch.build.targets.wheel]
exclude = [".ontos-internal/"]
```

### 8.3 Release Checklist Item
...
- [ ] Verify `.ontos-internal/` is NOT in sdist: `tar -tzf dist/*.tar.gz | grep -c ontos-internal` (should be 0)
- [ ] Verify `.ontos-internal/` is NOT in wheel: `unzip -l dist/*.whl | grep -c ontos-internal` (should be 0)
- [ ] Test fresh install in clean venv doesn't contain internal docs
```
...
- name: Verify no internal docs in sdist
  run: |
    if tar -tzf dist/*.tar.gz | grep -q ".ontos-internal"; then
      echo "ERROR: .ontos-internal/ found in sdist"
      exit 1
    fi

- name: Verify no internal docs in wheel
  run: |
    if unzip -l dist/*.whl | grep -q ".ontos-internal"; then
      echo "ERROR: .ontos-internal/ found in wheel"
      exit 1
    fi
```
...
## Part 9: Review Questions for AI Agents
...
**Codex's additional guardrails (Revision 1.4):**
- **Map provenance check:** Risk of publishing User Mode map from misconfigured Contributor environment. Added Section 6.4 with metadata header in generated maps (`<!-- Generated by Ontos | Mode: Contributor | ... -->`) and CI validation.
- **Release packaging hygiene:** Risk of `.ontos-internal/` shipping in distribution artifacts. Added Part 8 with defense-in-depth exclusions (`.gitattributes`, `MANIFEST.in`, `pyproject.toml`), release checklist, and CI verification.
