...
2. **Print file path:** (so agent knows what to edit)
   ```
   📝 Enhancing: .ontos-internal/logs/2025-12-15_feature-login.md
      Status: auto-generated → active (after you fill in details)
   ```
...
5. **Update status field:**
   ...
   status: auto-generated  →  status: active
   ```
...
Saving configuration...
  ✓ Mode changed: automated → prompted
  ✓ Preserved: LOG_RETENTION_COUNT = 25
  ✓ Preserved: MY_CUSTOM_SETTING = "foo" (unknown)
...
```
1. 9am:  Push on feat/login → creates 2025-12-15_login.md (branch: feat/login)
2. 11am: Push on fix/login  → slug is also "login"
...
- *v1.4: 2025-12-15 — Revised after v4 review (--enhance spec, collision clarity, missing section fallback, --reconfig preservation, BRANCH_PREFIX_MAP softened)*
