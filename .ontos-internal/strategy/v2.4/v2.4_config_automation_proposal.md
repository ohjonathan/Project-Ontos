...
**Date:** 2025-12-14 (Revised after architectural review)
**Status:** Draft - Revised based on Claude/Codex/Gemini feedback
**Version:** 1.1
...
- **Consolidation in Pre-Push** (from Gemini) - Automated users actually get automation
...
### 3.4 New: Consolidation in Pre-Push Hook

**Why:** Gemini identified that automated users won't run `Maintain Ontos`, so consolidation never happens.

**Solution:** Pre-push hook checks log count and triggers consolidation if needed.

```python
# In ontos_pre_push_check.py (automated mode only)

if AUTO_CONSOLIDATE and count_active_logs() > LOG_RETENTION_COUNT:
    print(f"📦 Auto-consolidating {count} logs older than {threshold} days...")
    run_consolidation(auto=True)
```

**Why not prompt instead of auto-consolidate?**

In automated mode, the user has explicitly chosen "zero friction." Prompting contradicts their choice. If they wanted prompts, they'd choose `prompted` mode.
...
**Why skip instead of prompt?**

In automated mode, prompting contradicts the "zero friction" promise. Better to skip gracefully and let user handle it.
...
| File | Changes |
|------|---------|
| `ontos_config_defaults.py` | Add `ONTOS_MODE`, `AUTO_ARCHIVE_ON_PUSH`, mode presets |
| `ontos_lib.py` | Add `resolve_config()` function (not new file) |
| `ontos_init.py` | New installation prompts, `--reconfig`, `--non-interactive` |
| `ontos_end_session.py` | Add `--auto` flag, session appending logic, `status: auto-generated` |
| `ontos_pre_push_check.py` | Auto-archive logic, consolidation trigger, dirty git check |
...
| File | Purpose |
|------|---------|
| `.ontos/templates/ontos_config.py.template` | Config file template |
...
**Proceed with implementation.** The revised design addresses all critical concerns:

1. ✅ Ghost log problem → Session appending
2. ✅ Quality signal → `status: auto-generated`
3. ✅ Silent failures → Defined fallback chain
4. ✅ Consolidation gap → Pre-push hook trigger
5. ✅ CI support → `ONTOS_SOURCE` env var + `--non-interactive`
...
| Consolidation in pre-push | Gemini | Trigger when log count > threshold |
...
## Appendix B: Session Appending Implementation Detail
...
def find_existing_log_for_today(branch_slug: str) -> Optional[str]:
    ...
    if matches:
        # Return the base log (not -2, -3 variants)
...
def append_to_log(log_path: str, new_commits: list):
    ...
    if marker in content:
        # Append new commits
        new_history = format_commits(new_commits)
    ...
    print(f"📝 Appended {len(new_commits)} commits to {log_path}")
```

---

*Document revised 2025-12-14 after architectural review by Claude, Codex, and Gemini.*
