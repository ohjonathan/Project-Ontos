...
**Behavior:**
1. Generate `CLAUDE.md` at repo root (or specified path)
2. Content includes:
   - Ontos activation reminder
   - Command to run: `python3 ontos.py map` (v2) or `ontos map` (v3)
   - Brief Ontos purpose description
3. Exit 0 on success, 1 if file exists (use `--force` to overwrite)
