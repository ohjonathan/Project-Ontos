---
id: v3_0_technical_architecture
type: strategy
status: active
...
| 1.2 | 2026-01-12 | Incorporated Round 1 peer review feedback (4 reviewers) |
| 1.3 | 2026-01-12 | Added Session Bootstrap feature (`ontos export` + v4.0 extension) |
| 1.4 | 2026-01-12 | Incorporated Round 2 feedback: interface completeness, file safety, PATH fallback |
...
This architecture was reviewed through two rounds of peer review by 4 architects.

**Round 1:** Identified interface gaps, consistency issues, and migration concerns. Resulted in v1.1 → v1.3 with two new features added (Global CLI + Local Data, Session Bootstrap).

**Round 2:** Validated new features, verified Round 1 resolutions. Resulted in v1.4 (this version).

**Final Verdict:** 4/4 reviewers approved for implementation (1 full approve, 3 approve with minor changes — all changes incorporated).

**Key changes from Round 2:**
- Added `find_project_root()` interface for CWD independence
- Improved shim hook reliability with `sys.executable -m ontos` fallback
- Removed v2 command references from export template
- Added file safety defaults for `ontos export`
- Marked `paths.py` for refactoring (uses `__file__` inappropriately)
- Added hook collision detection to `init` (warn and skip if existing)
- Added uninstallation guidance (Section 2.7)
- Documented nested repository handling (Section 2.6)

**Scope Note:** `ontos export` is a v3.0 scope expansion from Strategy Q2 (which originally deferred export templates to v4.0). This is a minimal bootstrap feature; full template system remains v4.0.

**Related Documents:**
- `V3.0-Implementation-Roadmap.md` — Version-by-version implementation plan
- `R2/CONSOLIDATED_R2_Review.md` — Round 2 peer feedback
- `R2/Chief_Architect_R2_Response.md` — Architect's response to Round 2
- `R1/CONSOLIDATED_Review.md` — Round 1 peer feedback
- `R1/Chief_Architect_Response.md` — Architect's response to Round 1
...
9. **Nested Repository Handling**: In nested repository scenarios (git submodules, monorepos with multiple Ontos configs), the CLI operates on the **innermost** `.ontos.toml` found walking up from CWD. Each submodule with its own `.ontos.toml` is treated as a separate Ontos instance.

### 2.7 Uninstallation Behavior

When user runs `pip uninstall ontos`:

**What Gets Removed:**
- Global `ontos` CLI binary
- Python package from site-packages

**What Remains (per-repo data):**
- `.ontos.toml` configuration file
- `Ontos_Context_Map.md` (generated output)
- All documentation in `docs/` or `.ontos-internal/`
- Session logs
- Git hooks (shim files in `.git/hooks/`)

**Git Hook Behavior After Uninstall:**
- Shim hooks will attempt to invoke `ontos`
- `FileNotFoundError` will trigger warning: "ontos not found in PATH. Skipping."
- Hook exits with code 0 (doesn't block git operations)
...
# Keep or remove documentation as desired
# rm -rf docs/  # Only if you want to remove all Ontos-managed docs
```
...
│   ├── stub.py              # `ontos stub`
│   ├── export.py            # `ontos export` — Session Bootstrap (NEW)
...
| `paths.py` | **REFACTOR** | ~300 | Path resolution and directory helpers. *Requires refactoring: uses `__file__`-relative paths that fail in global CLI context. Must accept injected `repo_root`.* |
...
**Purpose:** File system operations for document scanning, reading, and project root detection.
...
def find_project_root(start_path: Path) -> Path:
    ...
    1. Nearest `.ontos.toml` file walking up from start_path
    2. Git repository root (`.git/` directory)
    3. Raises FileNotFoundError with actionable hint if neither found

    This enables CWD independence per Constraint 2.6.8.
    """
...
def write_text_file(path: Path, content: str, encoding: str = "utf-8") -> None:
    ...
    For simple one-off writes (like CLAUDE.md from export).
    For transactional multi-file writes, use SessionContext instead.
    """
```
...
3. **Detect legacy `.ontos/scripts/` directory** — If found, warn: "Legacy Ontos v2 scripts detected. Consider removing `.ontos/` after migration."
4. Create `.ontos.toml` with smart defaults
5. **Install shim hooks with collision safety:**
   - For each hook (`pre-push`, `pre-commit`):
     - If hook file doesn't exist: Install shim
     - If hook exists and IS Ontos shim: Replace (update)
     - If hook exists and NOT Ontos shim: **Warn and skip** (don't overwrite)
       - Print: "Existing hook detected at .git/hooks/pre-push. Skipping. Use `--force` to overwrite."
   - `--force` flag: Overwrite existing hooks regardless
...
- 3: Hooks skipped due to existing non-Ontos hooks (partial success)
...
**Purpose:** Generate agent bootstrap files (CLAUDE.md, AGENTS.md) to ensure Ontos activation in AI sessions. Addresses the "Session Bootstrap" problem where AI assistants may forget about Ontos during long sessions or after context compaction.

**v3.0 Scope (Stub):**
- Single template: CLAUDE.md with hardcoded Ontos activation instructions
- No customization options
- Prepares extension point for v4.0 template system

**Public Interface:**
```python
def run(config: OntosConfig, output_type: str = "claude-md", force: bool = False) -> int:
    """Generate agent bootstrap file. Returns exit code."""
```

**Behavior:**
1. Determine output path (default: `{repo_root}/CLAUDE.md`)
2. **File safety check:**
   - If file exists and `--force` not provided: **Abort with message**
     - Print: "CLAUDE.md already exists. Use `--force` to overwrite, or edit manually."
     - Exit code 1
   - If file exists and `--force` provided: Overwrite
   - If file doesn't exist: Create
3. Generate content (v3-only syntax):
   - Ontos activation reminder
   - Commands: `ontos map`, `ontos log` (v3 syntax only — no v2 references)
   - Brief Ontos purpose description
4. Write file via `io/files.py:write_text_file()`
5. Exit 0 on success
...
**v4.0 Extension:** Template system with `ui/formatters/` pattern enabling customizable templates for different AI assistants.
...
### 5.6 `ontos export` — Agent Bootstrap Generation

Addresses the **Session Bootstrap** problem: AI assistants may "forget" about Ontos during long sessions or after context compaction. This command generates bootstrap files that instruct AI agents to activate Ontos.

```
User runs: ontos export [--type claude-md] [--output PATH] [--force]
           │
           ▼
┌─────────────────────────────────────────────────────┐
│ commands/export.py                                   │
...
│     └── Exit 1 if exists and no --force             │
│  4. Generate content from hardcoded template        │
│  5. Write file via SessionContext                   │
└───────────┬─────────────────────────────────────────┘
            │
            ▼
┌─────────────────────────────────────────────────────┐
│ Output: CLAUDE.md (or specified path)                │
│                                                      │
│ Template Content:                                    │
│ ┌─────────────────────────────────────────────────┐ │
│ │ # CLAUDE.md                                     │ │
...
Exit Codes:
  ...
  1: File exists (use --force to overwrite)
  2: Configuration error
```

**v4.0 Extension:** Template selection (`--type agents-md`, `--type cursor`) and customizable templates via `ui/formatters/`.
...
| **Agent Bootstrap Export** | `ui/formatters/` | `commands/export.py` stub with hardcoded CLAUDE.md template |

### Agent Bootstrap Export (Session Bootstrap Problem)

AI assistants may "forget" about Ontos during long sessions or after context compaction. Without an explicit activation mechanism, there's no guarantee that Ontos will be used consistently across sessions.

**v4.0 Agent Bootstrap Export** addresses this by:
- Generating `CLAUDE.md`, `AGENTS.md`, or custom agent instruction files
- Supporting customizable templates via `ui/formatters/`
- Auto-generating from project configuration and detected patterns
- Integrating with `ontos init` to optionally create bootstrap files

**v3.0 Foundation:** The `ontos export` command provides a minimal stub with a single hardcoded CLAUDE.md template. This establishes the command interface and file generation pattern that v4.0 will extend with template selection and customization.
...
| Term | Definition |
|------|------------|
| **Agent Bootstrap Export** | v4.0 feature for generating agent instruction files (CLAUDE.md, AGENTS.md) with customizable templates |
...
| **Session Bootstrap** | Mechanism ensuring AI agents activate Ontos at the start of each session, addressing context loss from compaction or long sessions |
...
| V3.0-Implementation-Roadmap.md | Version-by-version implementation plan |
| R1/CONSOLIDATED_Review.md | Round 1 merged peer feedback |
| R1/Chief_Architect_Response.md | Round 1 architect response |
| R2/CONSOLIDATED_R2_Review.md | Round 2 merged peer feedback |
| R2/Chief_Architect_R2_Response.md | Round 2 architect response
...
*End of Technical Architecture Specification v1.4*

*Approved for Implementation — 2026-01-12*

*Final version by Chief Architect (Claude Opus 4.5) — 2026-01-12*
*Reviewed by: Codex (A), Claude (B), Gemini Architectural (C), Gemini DeepThink (D)*
*Review rounds: 2 | Final verdict: 4/4 approved*
